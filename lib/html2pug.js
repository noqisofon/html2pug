...
Parser = (function(){
  ...
  function Parser(options){
    this.options = options != null
      ? options
      : {};
    debug('Parser#constructor ( %j )', this.options);
  }
...
Writer = (function(){
  ...
  function Writer(options){
    ...
    this.options = options != null
      ? options
      : {};
    debug('Writer#constructor( %j )', this.options);
    this.wrapLength = (ref$ = this.options.wrapLength) != null ? ref$ : 80;
    this.scalate = (ref$ = this.options.scalate) != null ? ref$ : false;
    this.attrSeparator = this.scalate || this.options.noAttrComma ? ' ' : ', ';
    debug('@attr-separator: %j', this.attrSeparator);
  ...
  Writer.prototype.tagAttribute = function(node, indents){
    ...
    if (!attrs || attrs.length === 0) {
      ...
      debug('results.join(%j): %j', this.attrSeparator, results.join(this.attrSeparator));
      if (results.length > 0) {
  ...
  Writer.prototype.buildTagAttr = function(attrName, attrValue){
    debug('Writer#build-tag-attr( %j, %j )', attrName, attrValue);
  ...
  Writer.prototype.writeTextContent = function(node, output, options){
    ...
    this.forEachChild(node, function(child){
      return this$.writeText(child, output, options);
    });
    return output.leave();
  };
...
Converter = (function(){
  ...
  function Converter(options){
    ...
    this.options = options != null
      ? options
      : {};
    debug('Converter#constructor( %j )', this.options);
    this.scalate = (ref$ = this.options.scalate) != null ? ref$ : false;
  ...
  Converter.prototype.writeScript = function(node, output, tagHead, tagAttr){
    debug('Converter#write-script( %j, %j, %j, %j )', node, output, tagHead, tagAttr);
    if (this.scalate) {
  ...
  Converter.prototype.writeStyle = function(node, output, tagHead, tagAttr){
    debug('Converter#write-style( %j, %j, %j, %j )', node, output, tagHead, tagAttr);
    if (this.scalate) {
      output.writeln(':css');
      this.writer.writeTextContent(node, output, {
        pipe: false,
...
function applyOptions(options){
  ...
  debug('ent-options.use-named-references: %j', entOptions.useNamedReferences);
